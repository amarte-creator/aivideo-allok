...
export default function CheckoutPage() {
  ...
  const generateWhatsAppMessage = () => {
    ...
    if (order.modelingAddOn) {
      ...
      message += `🎨 AI Content Creation: ${order.modelingAddOn.name} - ${modelingPrice}\n`
    }

    if (order.renderPackage) {
      const renderPrice = formatPrice(order.renderPackage.price)
      message += `🖼️ AI Images: ${order.renderPackage.name} (${order.renderPackage.quantity} images) - ${renderPrice}\n`
    }
  ...
  const getStepContent = () => {
    if (currentStep === 1 && !order.package?.includes3DModeling) {
      return {
        ...
        subtitle: "We need to know if you have reference images or if we should create everything from scratch.",
        options: [
          {
            id: "yes",
            title: "Yes, I have references",
            emoji: "✅",
            action: () => {
    ...
    if (currentStep === 2 && !order.package?.includes3DModeling && !order.has3DModel) {
      return {
        title: "Choose content complexity",
        subtitle: "Select the level that matches your creative requirements.",
        options: [
          {
            name: "Simple",
            price_inr: orderConfig.modelingOptions.simple.price_inr,
            price_usd: orderConfig.modelingOptions.simple.price_usd,
            complexity: "Basic compositions, standard styles",
            emoji: "🔷",
          },
          {
            name: "Medium",
            ...
            complexity: "Detailed scenes, custom styling",
            emoji: "🔶",
          },
          {
            name: "Complex",
            ...
            complexity: "Advanced compositions, unique art direction",
            emoji: "💎",
          },
        ].map((option) => {
    ...
    const isRenderStep =
      (currentStep === 1 && order.package?.includes3DModeling) ||
      (currentStep === 2 && order.package?.includes3DModeling) ||
      (currentStep === 3 && !order.package?.includes3DModeling)

    if (isRenderStep) {
      return {
        title: "Need additional AI images?",
        subtitle: "High-quality still images perfect for social media and marketing.",
        options: [
          {
            id: "yes",
            title: "Yes, add images",
            emoji: "🖼️",
            action: () => {
    ...
    if (order.needsRenders && !order.renderPackage) {
      return {
        title: "Choose image package",
        subtitle: "Select the number of high-quality AI images you need.",
        options: [
          {
            name: "Basic Pack",
          ...
          return {
            id: option.name,
            title: option.name,
            subtitle: `${option.quantity} images`,
            price: priceDisplay,
            emoji: option.emoji,
            action: () => {
              updateOrder({
                ...
                <CardContent className="p-5 space-y-4 sm:p-8 sm:space-y-6">
                  <div className="flex justify-between items-center py-3 border-b border-neutral-800 sm:py-4">
                    <div>
                      ...
                      <p className="text-neutral-400 text-sm mt-0.5 sm:mt-1">AI Video Package</p>
                    </div>
                    ...
                    <div className="flex justify-between items-center py-3 border-b border-neutral-800 sm:py-4">
                      <div>
                        <h4 className="font-semibold text-white text-sm sm:text-base">
                          AI Content Creation - {order.modelingAddOn.name}
                        </h4>
                  ...
                  {order.renderPackage && (
                    <div className="flex justify-between items-center py-3 border-b border-neutral-800 sm:py-4">
                      <div>
                        ...
                        <p className="text-neutral-400 text-xs mt-0.5 sm:text-sm sm:mt-1">
                          {order.renderPackage.quantity} images
                        </p>
                      </div>
                      <span className="font-bold text-white text-sm sm:text-base">
